  ...
  const userOptions =
    users?.map((user) => ({
      ...
      label: `${user.first_name} ${user.last_name} (${
        user.email
      }) - ${getUserMatchCount(user.id)} matches`,
    })) ?? [];
      ...
      try {
        await client.transact(async (tx) => {
          ...
          for (const match of stats.matches) {
            isPlayer1 = match.player_1 === value.oldUserId;
          ...
          for (const game of stats.games) {
            await tx.update("games", game.id, (g) => {
              const p1Win = g.player_1_score > g.player_2_score;
              const isWinner = isPlayer1 ? p1Win : !p1Win;
              if (isWinner) g.winner = value.newUserId;
            });
          }
