...
> {
  try {
    ...
    const leaderboardInfo = prevEventDetails?.topPlayers
      ? `Top players: ${prevEventDetails.topPlayers
          .map((player) => `${player.first_name} ${player.last_name}`)
          .join(", ")} Previous event matches: ${JSON.stringify(
          prevEventDetails.prevEventMatches
        )}`
      : "";
...
Time: ${format(previousEvent.start_time, "HH:mm")} - ${format(
...
- name: "${
      previousEvent.name
    }" (keep exactly the same format, just update the date)
- description (string, incorporating historical events and player dynamics)
- start_time (ISO string for ${format(nextDate, "EEEE do MMMM")} at ${format(
      previousEvent.start_time,
      "HH:mm"
    )})
- end_time (ISO string for ${format(nextDate, "EEEE do MMMM")} at ${format(
      previousEvent.end_time,
      "HH:mm"
    )})
